...
class MyVehicle:
    def __init__(self, dataset, num_ptc=100, do_texture=True, particle_shift=False, name=""):
        self.name = name
        if self.name != "":
            self.name = name + "_"
    ...
    def run_main(self):
        ...
        for idx_encoder in range(201, self.encoder_stamps.shape[0]):
            if idx_encoder == 600:
            ...
            map_corr = self.map_correlation_mat_version(ranges, xy_is, xs=xs, ys=ys)  # 100 * 81

            corr_maxs = np.max(map_corr, axis=1)  # 100 * 1
            print("corr_max", np.max(corr_maxs))
            # if np.max(corr_maxs) > 10000:
            #     self.input_noise = [[0, 0.1], [0, 0.1]]
            # elif np.max(corr_maxs) > 5000:
            #     self.input_noise = [[0, 0.1], [0, 0.3]]
            # else:
            #     self.input_noise = [[0, 0.1], [0, 0.5]]

            corr_maxs = corr_maxs - np.max(corr_maxs)

            particle_best_corrs = np.argmax(map_corr, axis=1)  # 100 * 1 best shift for each article
...
if __name__ == '__main__':
    # ts = tic()
    # vel = MyVehicle(dataset=21, do_texture=True, particle_shift=True, name="particle_shift")
    # vel.run_main()
    # toc(ts, "dataset 21 using")
    #
    # ts = tic()
    # vel = MyVehicle(dataset=20, do_texture=True, particle_shift=True, name="particle_shift")
    # vel.run_main()
    # toc(ts, "dataset 20 using")
    #
    ts = tic()
    vel = MyVehicle(dataset=23, num_ptc=300, do_texture=False, particle_shift=True, name="particle_shift_noise1")
    vel.run_main()
    toc(ts, "dataset 20 using")
